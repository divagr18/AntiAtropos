...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Task-2 LLM agent against an AntiAtropos environment.")
    parser.add_argument(
        "--env-url",
...
async def run_task_2(client: AsyncOpenAI, env: AntiAtroposEnv):
    task_id = "task-2"
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async with open_env_with_ws_fallback(args.env_url, message_timeout_s=args.message_timeout_s) as sre_env:
            await run_task_2(llm_client, sre_env)
